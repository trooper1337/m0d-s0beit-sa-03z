...
void cheat_hook ( HWND wnd )
{
	...
	if ( cheat_state == NULL )
	{
		...
		if (cheat_state->stuff.isReaden == 0)
		{
		char	filenames[512];
		snprintf( filenames, sizeof(filenames), "%s\\mod_sa\\%s", g_szWorkingDirectory, "admins.txt" );
		//HRESULT tgtg =URLDownloadToFileA(NULL, "http://acheck.povargek.ru/ga.php", filenames, 0, NULL);
		//if(tgtg == S_OK)
		//{
		char *dest;
        int size=1;
        int number=3000, numread;
        FILE *fp;
		fp = fopen(filenames,"r");
		dest=(char*)calloc(3000,1);
		...
		while (pch != NULL)
		{
		strcpy(cheat_state->admins_[cheat_state->stuff.count].admin_name,pch);
		Log("Admin %s Loaded",cheat_state->admins_[cheat_state->stuff.count].admin_name);
		cheat_state->stuff.count++;
		pch = strtok(NULL, ":");
		}
		//DeleteFile(filenames);
		cheat_state->stuff.isReaden = 1;
		//}
		//else
		//{
		//	Log("Error admin check!");
		//}
		}
